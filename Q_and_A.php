<?php 
	//QAのやりとりを表示する画面。URLは店側から顧客へのメールに記載される
	//URLのパラメータで、店側からのアクセスか、顧客からのアクセスかを判定する。
	//アクセスの際に、セッションに問合せ番号とアクセス元を保存する。ajax_get_QAtalk.phpはこのセッションをもとに、やりとりの内容を返す。
	if(empty($_GET)){
		echo "見せないよ！er1";
		exit();
	}
	...
	//$_SESSION["sts"] = $_GET["sender"];        //暗号
	$sender = rot13decrypt2($_GET["sender"] ?? "");//Q or A
	U::log("\$sender",$sender,4);

	$include_header_tag_path = ($sender==="shop") ? "header_tag_admin.php" : "header_tag.php";
	if($sender==="customer"){//お客さん用のURLから来た場合
		$user_hash = "";
		$_SESSION["user_id"] = "%";
	}else if($sender==="shop"){//お客さんに答える。もしくは問い返す
		$user_hash = $_GET["key"] ;
		$_SESSION["user_id"] = rot13decrypt2($user_hash);

	}else{
		echo "見せないよ！er2:".$sender;
		exit();
	}
...
<BODY>
	...
	<?php 
		include $include_header_tag_path;
	?>
		...
		<div class='row pb-3 pt-3'>
			<div class='col-12 col-md-10'>
				<template v-for='(list,index) in talk' :key='list.seq'>
					<div v-if='list.sender==="customer"' class="balloon-chat left">
						<!-- 左の吹き出し -->
					...
					<div v-if='list.sender==="shop"' class="balloon-chat right">
						<!-- 右の吹き出し -->
		...
		<div class='row' >
			<div class='col-12 col-md-10'>
				...
				<div class='col-2 p-0 position-relative'>
					<button type='button' @click='send_msg("<?php echo $sender;?>")' class='btn btn-primary position-absolute top-50 translate-middle-y' style='width:100%;height:60px;'>送信</button>
				</div>
				</div>
			</div>
		</div>
	...
	<script>
		...
		createApp({//販売画面
			setup() {
				...
				const get_talk = () =>{
					axios.get('ajax_get_QAtalk.php?key=<?php echo $sender;?>')
					.then((response)=>{
				...
				const send_msg = (p_sender) =>{
					loader.value=true
					const form = new FormData();
					let send_program = ""
					if(p_sender==="customer"){
						form.append(`key`, "customer")
						send_program = "ajax_sendmail_from_C.php"
					}else if(p_sender==="shop"){
						form.append(`key`, "shop")
						send_program = "ajax_sendmail_from_B.php"
					}else{
						alert("送信失敗：想定外のアクセスです。")
						return;
					}
					...
					//form.append(`subject`, `RE:【${talk.value[0].yagou}】${talk.value[0].shouhinNM}」`)
					form.append(`subject`, `${talk.value[0].shouhinNM}`)
					//form.append(`mailbody`, `※このメールは送信専用です。※\n\n${message.value}`)
					form.append(`mailbody`, message.value)
					form.append(`action`, 'kaitou')//action:toiawase:初回の問合せ or kaitou:それぞれからの'回答'（初回問合せは客の場合indexphpから、店の場合は受注管理画面からなので、actionは必要ないが、念のためつける）
					//form.append(`qa_text`, message.value)
					//form.append(`sts`, "session")
					form.append(`csrf_token`, token)

					axios.post(send_program,form, {headers: {'Content-Type': 'multipart/form-data'}})
					.then((response)=>{
