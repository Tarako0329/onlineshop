<?php
//PGNAME:ajax_sendmail_from_B.php
//概要：通販サイトの出店者が受注管理画面から顧客へ質問をした際に、顧客へメールで通知するためのAJAX処理。質問内容はDBにも保存される。
//パターンは2パターンあり
...
if($rtn !== true){
	...
	$reseve_status = true;
		...
		try{
			...
			$sender = "shop";
			if($action==="kakunin"){
				//問合せNoの取得（念のため、同じ受注に対して確認メールを出してないか確認。出していた場合は同じ問合せ番号を利用する）
			...
			//顧客回答用のURL
			$URL = ROOT_URL."Q_and_A.php?askNO=".rot13encrypt2($askNO)."&sender=".rot13encrypt2("customer");
			
			//メール送信
			$send_rtn = false;

			if($action==="kakunin"){//受注管理画面のお客様向け質問（店⇒客）
				$head = $yagou." より確認のお問い合わせがあります。\r\n"
						."ご回答いただく場合は\r\n".$URL."\r\nよりお願いします\r\n\r\n====以下、".$yagou." より====\r\n\r\n";
			...
			$send_rtn = U::send_mail($CusMailAdd,$subject, $head.$params["body"]."\n\n※このメールは送信専用です。返信はメール上部のURLよりお願いします。※\n", APP_NAME,"");

			//出店者へのCC
			$head = "下記内容にてメールを送信しました。\r\n========\r\n";
			if(U::exist($lineID)){
				$ShopMailAdd = "LINE";
				$send_cc_rtn = U::send_line($lineID,$head."【".$subject."】\r\n".$params["body"]);//出店者へお知らせLINE
			}else{
				$send_cc_rtn = U::send_mail($ShopMailAdd,$subject,$head.$params["body"],APP_NAME,"");//出店者へお知らせメール
			}
