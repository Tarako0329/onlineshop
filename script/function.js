...
const UPLOADFILE = (id,filesubname,csrf_token)=>{//サイト設定情報取得
	return new Promise((resolve, reject) => {
		UPLOADFILE_SHORI(id,filesubname,csrf_token,resolve);
	});
}
const UPLOADFILE_SHORI = (id,filesubname,csrf_token,resolve) =>{//写真アップロード処理・写真をアップしファイルパスを取得
