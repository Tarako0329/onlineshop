...
const configration = (Where_to_use,p_token,p_hash) => createApp({//サイト設定
  setup() {
    ...
    const fb_id = ref('')
    const x_id = ref('')
    const mail_body_auto_ini = '<購入者名> 様\n\nご注文ありがとうございます。\n以下の内容にて、ご注文を受け付けました。\n\n<注文内容>\n\n<購入者情報>\n\n<届け先情報>\n\n※弊社担当にてご注文内容の確認が取れましたら、お支払い・納期等についてのご案内メールを送付いたします。\n※メールが届かない場合、また、不明点・お問い合わせ等ございましたら以下までご連絡くださいませ。\n\n*************************\n<自社名>\n<自社住所>\nTEL:<問合せ受付TEL>\nMAIL:<問合せ受付MAIL>\n*************************'
    const mail_body_ini = '<購入者名> 様\n\nご注文ありがとうございます。\n以下の内容にて、ご注文を承りました。\n\n<送料込の注文内容>\n\n<購入者情報>\n\n<届け先情報>\n\nお支払いが確認できましたら発送準備に入ります。\n<支払方法>\n振込手数料についてはお客様負担となります\n\n不明点・お問い合わせ等ございましたら下記へご連絡ください。\n\n*************************\n<自社名>\n<自社住所>\nTEL:<問合せ受付TEL>\nMAIL:<問合せ受付MAIL>\n*************************'
    const mail_body_paid_ini = '<購入者名> 様\n\nいつもありがとうございます。\n\n以下のご注文についてのお支払いを確認いたしました。\n<領収書LINK>\n\n発送が終わりましたら再度ご連絡させていただきます。\n\n<送料込の注文内容>\n\n<購入者情報>\n\n何かございましたら以下までご連絡くださいませ。\n\n*************************\n<自社名>\n<自社住所>\nTEL:<問合せ受付TEL>\nMAIL:<問合せ受付MAIL>\n*************************'
    const mail_body_sent_ini = '<購入者名> 様\n\nいつもありがとうございます。\n\n以下のご注文について、本日商品を発送いたしました。\n\n<注文内容>\n\n<購入者情報>\n\n<届け先情報>\n\n（お届け先未記載の場合は<購入者名> 様宛にお送りしてます。）\n\n<配送状況>\n\n商品のご到着までしばらくお待ちください。\n\n今後とも <自社名> をよろしくお願いします。\n\n*************************\n<自社名>\n<自社住所>\nTEL:<問合せ受付TEL>\nMAIL:<問合せ受付MAIL>\n*************************'
    const mail_body_cancel_ini = '<購入者名> 様\n\nいつもありがとうございます。\n\n以下のご注文について、キャンセルを受け付けました。\n\n<注文内容>\n\n<購入者情報>\n\n今後とも <自社名> をよろしくお願いします。\n\n*************************\n<自社名>\n<自社住所>\nTEL:<問合せ受付TEL>\nMAIL:<問合せ受付MAIL>\n*************************'
    const mail_body_auto = ref(mail_body_auto_ini)
    const mail_body = ref(mail_body_ini)
    const mail_body_paid = ref(mail_body_paid_ini)
    const mail_body_sent = ref(mail_body_sent_ini)
    const mail_body_cancel = ref(mail_body_cancel_ini)
    const site_name = ref('')
    const site_pr = ref('')
    const logo = ref('')
    ...
    const mail_temp_ins = (id,val) =>{
      ...
      if(id==="mail_body_auto"){
        mail_body_auto.value = sentence
      }else if(id==="mail_body"){
        mail_body.value = sentence
      }else if(id==="mail_body_paid"){
        mail_body_paid.value = sentence
      }else if(id==="mail_body_sent"){
        mail_body_sent.value = sentence
      }else if(id==="mail_body_cancel"){
        mail_body_cancel.value = sentence
      }
    }
    const mail_temp_Initializer = (id) =>{
      if(id==="mail_body_auto"){
        mail_body_auto.value = mail_body_auto_ini
      }else if(id==="mail_body"){
        mail_body.value = mail_body_ini
      }else if(id==="mail_body_paid"){
        mail_body_paid.value = mail_body_paid_ini
      }else if(id==="mail_body_sent"){
        mail_body_sent.value = mail_body_sent_ini
      }else if(id==="mail_body_cancel"){
        mail_body_cancel.value = mail_body_cancel_ini
      }
    }
    const line_test = () =>{
