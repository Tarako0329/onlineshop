<?php
	require "php_header.php";
	//$rtn = true;//csrf_checker(["payment.php"],["G","C","S"]);
	/*$rtn = csrf_checker(["payment.php"],["G","C","S"]);
	if($rtn !== true){
		echo "不正アクセス。";
		exit();
	}*/
	$token = csrf_create();
	if(empty($_GET["key"])){
		echo "参照用のURLが異なります。";
		exit();
	}
	$user_hash = $_GET["key"] ;
	$_SESSION["user_id"] = rot13decrypt2($user_hash);

	log_writer2("\$_GET",$_GET,"lv3");

	$display_msg = "";

	try{
		$db->begin_tran();
		
		if($params["paytype"]==="stripe"){
			$params["uid"] = $_SESSION["user_id"];
			$params["orderNO"] = $_GET["orderNO"];
			$params["paytype"] = $_GET["paytype"];	//Stripe　の場合Stripe決済完了時のみ or done(済)：Stripe支払含め、支払が完了している場合
			
			$sqlstr_h = "UPDATE juchuu_head set payment = 2 where orderNO = :orderNO and uid like :uid";
			//受注ヘッダ登録

			$db->UP_DEL_EXEC($sqlstr_h,$params);
			
			//入金完了を出店者へ通知
			$owner = $db->SELECT("SELECT * from Users_online where `uid` = :uid",$params);

			$juchuu_head = $db->SELECT("SELECT juchuu_head.*,(本体額+postage) as 入金額 from juchuu_head 
				inner join (select orderNO,sum(goukeitanka+zei) as 本体額 from juchuu_meisai group by orderNO) as Meisai 
				on juchuu_head.orderNO = Meisai.orderNO 
				where juchuu_head.orderNO = :orderNO and juchuu_head.uid like :uid"
				,$params);

			$kokyaku = $juchuu_head[0]['name'];
			$orderno = $juchuu_head[0]['orderNO'];
			$nyukin = $juchuu_head[0]['入金額'];

			$body = <<<EOM
				$kokyaku 様よりクレジットカード決済による入金を確認しました。

				受付No：$orderno
				ご入金額：￥ $nyukin -

				受注管理画面の入金ステータスを「入金済み」に変更しました。

				念のため、Stripe管理画面を確認してください。
				https://dashboard.stripe.com/payments
			EOM;

			$lineID =$owner[0]["line_id"];
			if(U::exist($lineID)){//LINEで通知
				$rtn = U::send_line($owner[0]["line_id"],$body);
			}else if(!empty($owner[0]["mail"])){
				$rtn = U::send_mail($owner[0]["mail"],"入金通知[No:".$juchuu_head[0]["orderNO"]."]",$body,APP_NAME." onLineShop",$owner[0]["mail"]);
			}

			//クレジット支払自動返信のONOFF設定を追加してから有効にする
			/*
			$ryoushu_url = ROOT_URL."pdf_receipt.php?hash=".$user_hash."&val=".$orderno."&tp=1";
			$body2 = <<<EOM
				$kokyaku 様よりクレジットカード決済によるお支払いを確認しました。

				受付No：$orderno
				ご入金額：￥ $nyukin -

				お支払いありがとうございました。
				商品の発送まで今しばらくお待ちください。

				領収書は下記ＵＲＬよりダウンロードをお願いいたします。
				$ryoushu_url
			EOM;
			$rtn = U::send_mail($owner[0]["mail"],"お支払い完了通知[No:".$juchuu_head[0]["orderNO"]."]",$body2,APP_NAME." onLineShop",$juchuu_head[0]["mail"]);
			*/
			$display_msg="<h1>ありがとうございます。</h1><h1>お支払いを受付いたしました。</h1>";
		}else{//支払済みの状態で請求画面を開いた場合
			$display_msg="<h1>ありがとうございます。</h1><h1>ご注文のお支払いは完了しております。</h1>";
		}
		$db->commit_tran();
		$reseve_status=true;
	}catch(\Throwable $e){
		$db->Exception_rollback($e);
		$display_msg = "システムエラーによる更新失敗。管理者へ通知しました。";
		$reseve_status=true;
	}


?>

<!DOCTYPE html>
<html lang='ja'>
<head>
    <?php 
    //共通部分、bootstrap設定、フォントCND、ファビコン等
    include "head_bs5.php" 
    ?>
    <style>
      .btn{
        min-width: 50px;
      }
    </style>
    <TITLE><?php echo APP_NAME;?></TITLE>
</head>
<BODY >
  <div id='app' style='min-height: 100%'>
  <?php include "header_tag.php"  ?>
  <MAIN class='container common_main' data-bs-spy="scroll" data-bs-target="#scrollspy">
		<?php echo $display_msg; ?>
	</MAIN>
	</div>
</BODY>
</html>